...
//        wchar_t arrStress[MAX_SIZE] = { 0 };
    ...
    void FillWordFormData(shared_ptr<CWordForm> spWf, StWordForm* pstTarget) 
    {
        ...
        pstTarget->eCase = spWf->eCase();
        pstTarget->eNumber = spWf->eNumber();
        pstTarget->eSubparadigm = spWf->eSubparadigm();
        pstTarget->eGender = spWf->eGender();
        pstTarget->ePerson = spWf->ePerson();
        pstTarget->eAnimacy = spWf->eAnimacy();
        pstTarget->eReflexivity = spWf->eReflexive();
        pstTarget->eAspect = spWf->eAspect();
        pstTarget->eStatus = spWf->eStatus();
        pstTarget->bIrregular = spWf->bIrregular();      // came from the DB as opposed to being generated by the app
        memcpy(pstTarget->szLeadComment, spWf->sLeadComment(), spWf->sLeadComment().uiLength() * sizeof(wchar_t));
        memcpy(pstTarget->szTrailingComment, spWf->sTrailingComment(), spWf->sTrailingComment().uiLength() * sizeof(wchar_t));
        pstTarget->bIsEdited = spWf->bIsEdited();
        pstTarget->bIsVariant = spWf->bIsVariant();
        memcpy(pstTarget->szGrammHash, spWf->sGramHash(), spWf->sGramHash().uiLength() * sizeof(wchar_t));
        CopyStressData(spWf, pstTarget->szStress);
